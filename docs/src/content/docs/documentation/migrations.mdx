...
Define the desired schema in `traildepot/schema/main.sql` and let TrailBase
...
Startup and `SIGHUP` reloads also run schema sync for `main.sql` when present.
...
- If `traildepot/schema/org.sql` is present,
