...
## Declarative Schema Workflow (Optional)

Besides creating migrations manually, TrailBase also supports an optional
declarative SQLite workflow.

In this mode, you define the desired schema in `traildepot/schema/main.sql`
and let TrailBase compute and materialize migrations for you.

### 1. Enable Declarative Mode

Set the config field in `traildepot/config/config.textproto`:

```textproto
schema_mode: DECLARATIVE
```

`APPEND` remains the default if this field is omitted.

### 2. Plan and Review Schema Drift
...
With `schema_mode: DECLARATIVE`, startup and `SIGHUP` reloads will also run the
declarative sync path.

<Aside type="note" title="Safety and Rollouts">
  Declarative mode still follows append-only migration history. Destructive
  schema changes require explicit opt-in flags when using `apply`.
</Aside>
