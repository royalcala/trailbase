...
fn parse_desired(sql: &str) -> Result<(Vec<Table>, Vec<TableIndex>), SchemaDiffError> {
    let stmts = parse_into_statements(sql)
        .map_err(|e| SchemaDiffError::Parse(format!("Parse error: {e}")))?;
...
fn diff_table_columns(
    ...
    for col in &desired.columns {
        ...
        ops.push(DiffOperation {
            ...
            is_supported: true,
            sql: format!(
                "ALTER TABLE \"{table_name}\" ADD COLUMN {col_fragment}"
            ),
        });
    }
...
mod tests {
    ...
    fn detects_create_table() {
        ...
        assert!(diff.operations[0].description.contains("CREATE TABLE 'users'"));
